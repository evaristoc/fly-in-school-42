...
|[v3-capacity_heuristic_high_traffic folder](./v3-capacity_heuristic_high_traffic)|Almost exactly the same as the previous one, except for small modified conditions for the heuristic calculation so it favours **high capacity / high traffic regions** instead. This project can solve some custom graphs better than the previous version, but largerly fails to solve the "impossible dream" exercise.|
