...
|[v5-cbs](./v5-cbs)|WORK IN PROGRESS -- This is a _first working draft_ of a **CBS (constraint-based search) method implementation** to solve the Fly-In. In this iteration, the **low-level planners implement the Dijkstra** of previous version. High level planner implements a **best first search** on a binary tree of branched constraint nodes. Differently to the previous versions where priority planner was implemented, the CBS makes use of **constraints** not as reservations but as **forbidden entries**. It was found that some of the requirements of the Fly-In might not correspond with typical classical cases for cbs implementations, like _capacities for zones **and** edges constraints_, resulting in the reassessment of the common method. There were other deviations to the classical cbs to fit the Fly-In exercise, but always respecting the cbs framework. This version works well so far for simple, straight graphs but it is still buggy and fails to takes too many cycles to complete graphs that are either too complex or the entering values are too high (eg. nb drones higher than 50), suggesting performance issues. (_NOTE: CBS was the original method I wanted to try for the Fly-In assignment before deciding to submit a priority planner_)|
|WORK IN PROGRESS -- Departure 01 | **Departures** will be versions holding modifications more related to the architecture or any other feature that is not strictly related to the solution itself. _In this departure_ I will focus on making the code **more scalable and modular** than previous versions, in order to facilate the morphing of the original code base into different solutions of the Fly-In problem.|
|[v6-cbs_hop_based](./v6-cbs_hop_based)|WORK IN PROGRESS -- This is the same as before except that the conflict detection function **ignores capacity**.|
