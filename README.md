...
|[v5-cbs](./v5-cbs)|(_This is still work in progress_) This is a _first working draft_ of a **CBS method implementation** to solve the Fly-In. In this iteration, the **low-level planners implement the Dijkstra** of previous version. High level planner implements a **best first search** on expanded constraint nodes. Differently to the previous versions where priority planner was implemented, the CBS makes use of **constraints** not as reservations but as **forbidden entries**. The CBS implementation is more complex than the priority planner, and some of the requirements of the Fly-In were not found to be typical classical cbs, requiring non-standard modifications. One example appeared to be the _capacities for zones **and** edges_. This current version works well for simple, straight charts but it is still buggy and fails to complete complex or circular graphs. (_NOTE: CBS was the original method I wanted to try for the Fly-In assignment before deciding to submit a priority planner_)|
